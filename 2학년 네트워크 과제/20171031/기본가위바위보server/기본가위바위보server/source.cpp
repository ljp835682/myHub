...
bool AddThread(_ClientInfo * ptr)
{
	EnterCriticalSection(&cs);						//hThread, ClientInfo, th_count 를 보호
...
void RemoveThread(int index)
{
	EnterCriticalSection(&cs);						//hThread, th_count 를 보호
...
_ClientInfo* AddClient(SOCKET sock, SOCKADDR_IN clientaddr)
{
	EnterCriticalSection(&cs);							//ClientInfo, cl_count 를 보호
